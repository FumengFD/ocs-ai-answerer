...
| Windows | CMD UTF-8 编码修复 + 启动横幅边框完美对齐 |
| 地址 | 启动信息显示 `127.0.0.1` 而非 `0.0.0.0`（可直接点击） |
| 密钥 | 访问密钥**每次启动都显示**，不再丢失 |
| 修复 | `npm.cmd` 兼容、Vue 构建 `const->let` 修复 |
| 预构建 | `dist/` 已包含在仓库中，clone 即可用 |
