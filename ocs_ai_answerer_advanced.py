...
MODEL_NAME = os.getenv('MODEL_NAME', 'deepseek-v4-flash')     # 模型名称
...
# 支持deepseek-v4-flash（普通模式）和deepseek-v4-pro（思考模式）
DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY', '')
DEEPSEEK_BASE_URL = os.getenv('DEEPSEEK_BASE_URL', 'https://api.deepseek.com')
DEEPSEEK_MODEL = os.getenv('DEEPSEEK_MODEL', 'deepseek-v4-flash')  # deepseek-v4-flash 或 deepseek-v4-pro
...
# max_tokens 限制:
# - deepseek-v4-flash: [1, 8192] (最大8K)
# - deepseek-v4-pro: [1, 65536] (最大64K)
# 普通模式的 max_tokens（默认500）
MAX_TOKENS_RAW = int(os.getenv('MAX_TOKENS', '500'))
MAX_TOKENS = max(1, min(8192, MAX_TOKENS_RAW))  # 默认限制到8K（deepseek-v4-flash的限制）

# 思考模式的 max_tokens（默认4096，可以更大以支持复杂推理）
REASONING_MAX_TOKENS_RAW = int(os.getenv('REASONING_MAX_TOKENS', '4096'))
REASONING_MAX_TOKENS = max(1, min(65536, REASONING_MAX_TOKENS_RAW))  # 限制到64K（deepseek-v4-pro的限制）
    ...
    try:
        ...
        if not raw_answer and model_client:
            ...
            if model_client.is_auto_mode:
                ...
                model_name = model_client.model if not force_reasoning else ('deepseek-v4-pro' if model_client.provider == 'deepseek' else model_client.model)
