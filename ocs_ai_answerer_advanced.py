...
# ==================== .env 自动创建 ====================
# 如果 .env 文件不存在，从 env.template 模板自动生成
# 兼容 PyInstaller 打包后的 exe 运行环境
if getattr(sys, 'frozen', False):
    _app_dir = os.path.dirname(sys.executable)
else:
    _app_dir = os.path.dirname(os.path.abspath(__file__))
_env_file = os.path.join(_app_dir, '.env')
_template_file = os.path.join(_app_dir, 'env.template')
if not os.path.exists(_env_file) and os.path.exists(_template_file):
    with open(_template_file, 'r', encoding='utf-8') as _src:
        with open(_env_file, 'w', encoding='utf-8') as _dst:
            _dst.write(_src.read())
    print("📝 首次运行：已从 env.template 自动创建 .env 文件")
    print("   请编辑 .env 填入 API 密钥后重启程序")

# 加载环境变量
load_dotenv()

# Windows CMD/终端 UTF-8 编码修复，确保 box-drawing 字符正确显示
...
MODEL_NAME = os.getenv('MODEL_NAME', 'deepseek-flash')     # 模型名称
...
# 支持deepseek-flash（普通模式）和deepseek-pro（思考模式）
DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY', '')
DEEPSEEK_BASE_URL = os.getenv('DEEPSEEK_BASE_URL', 'https://api.deepseek.com')
DEEPSEEK_MODEL = os.getenv('DEEPSEEK_MODEL', 'deepseek-flash')  # deepseek-flash 或 deepseek-pro
...
# max_tokens 限制:
# - deepseek-flash: [1, 8192] (最大8K)
# - deepseek-pro: [1, 65536] (最大64K)
# 普通模式的 max_tokens（默认500）
MAX_TOKENS_RAW = int(os.getenv('MAX_TOKENS', '500'))
MAX_TOKENS = max(1, min(8192, MAX_TOKENS_RAW))  # 默认限制到8K（deepseek-flash的限制）

# 思考模式的 max_tokens（默认4096，可以更大以支持复杂推理）
REASONING_MAX_TOKENS_RAW = int(os.getenv('REASONING_MAX_TOKENS', '4096'))
REASONING_MAX_TOKENS = max(1, min(65536, REASONING_MAX_TOKENS_RAW))  # 限制到64K（deepseek-pro的限制）
        ...
        if os.path.exists(self.key_file):
            ...
            try:
                with open(self.key_file, 'r', encoding='utf-8') as f:
                    ...
                    self._raw_key = data.get('raw_key', '')
                    logger.info(f"✅ 已加载现有访问密钥")
                    if self._raw_key:
                        logger.info(f"   访问密钥: {self._raw_key}")
            except Exception as e:
...
class ModelClient:
    ...
    def __init__(self, provider: str = MODEL_PROVIDER):
        ...
        if self.provider == 'auto':
            ...
            # 如果启用思考模式，使用deepseek-pro
            if self.enable_reasoning:
                self.model = 'deepseek-pro'
...
def save_to_csv(question: str, options: List[str], q_type: str, raw_answer: str, 
    ...
    try:
        ...
        with open(csv_file, 'a', newline='', encoding='utf-8-sig') as f:
            ...
            # 豆包-Seed-1.6-Thinking: 推理输入0.8元/百万tokens，推理输出2元/百万tokens
    ...
    try:
        ...
        if not raw_answer and model_client:
            ...
            if model_client.is_auto_mode:
                ...
                model_name = model_client.model if not force_reasoning else ('deepseek-pro' if model_client.provider == 'deepseek' else model_client.model)
...
def serve_spa(path):
    ...
    # dist 存在时使用 Vue SPA，否则回退到静态 config_panel.html
    if os.path.exists(dist_dir) and os.path.exists(index_file):
        try:
    ...
    # 回退：静态 config_panel.html
    config_html = os.path.join(os.path.dirname(__file__), 'config_panel.html')
    ...
    return jsonify({"error": "前端界面不可用"}), 503
...
def _display_width(s: str) -> int:
    """计算字符串在终端中的实际显示宽度（CJK/emoji 算2列，其余算1列）。"""
    width = 0
    for ch in s:
...
def _pad_to_display(s: str, target_width: int) -> str:
    """将字符串补空格到指定的显示宽度。"""
...
def _build_frontend() -> bool:
    """自动构建 Vue3 前端（若 Node.js 可用且 dist/ 不存在）。返回是否构建成功。"""
    import subprocess
    ...
    try:
        ...
        print("   📦 npm install...")
        subprocess.run(
            ['npm' + ('.cmd' if sys.platform == 'win32' else ''), 'install'],
            cwd=frontend_dir,
            check=True,
            timeout=300
        )
        ...
        subprocess.run(
            ['npm' + ('.cmd' if sys.platform == 'win32' else ''), 'run', 'build'],
            cwd=frontend_dir,
            check=True,
            timeout=120
        )
        ...
        print("⚠️  前端构建超时，请手动执行: cd frontend && npm run build")
        print()
        return False
    except FileNotFoundError:
        print("⚠️  未检测到 Node.js/npm，请安装 Node.js 后手动构建：")
...
if __name__ == '__main__':
    ...
    # 构建启动横幅 (使用显示宽度感知的辅助函数，确保 CJK/emoji 对齐)
    _TOP = "╔" + "═" * 55 + "╗"
    _SEP = "╠" + "═" * 55 + "╣"
    _BOT = "╚" + "═" * 55 + "╝"
    _CW = 51  # 内容区显示宽度 (═ 数 55 - 4 个内侧空格)

    def _box(content: str) -> str:
        """构建一行带对齐右边框的盒子内容。"""
        return "║  " + _pad_to_display(content, _CW) + "  ║"

    # 浏览器可访问的显示地址 (0.0.0.0 是监听地址，浏览器需用 127.0.0.1)
    _display_host = "127.0.0.1" if HOST == "0.0.0.0" else HOST

    # 检测前端模式
    _dist_index = os.path.join(os.path.dirname(__file__), "dist", "index.html")
    _frontend_mode = "🌐 Vue3 SPA" if os.path.exists(_dist_index) else "📄 静态 HTML"

    _banner_lines = [
        _TOP,
        _box("      OCS智能答题API服务 - 多模型支持版本 v3.0"),
        _SEP,
        _box(f"{_frontend_mode}: http://{_display_host}:{PORT}/"),
        _box(f"📊 数据可视化: http://{_display_host}:{PORT}/viewer"),
        _box(f"📖 API文档: http://{_display_host}:{PORT}/docs"),
        _SEP,
        _box(f"接口地址: http://{_display_host}:{PORT}/api/answer"),
        _box(f"健康检查: http://{_display_host}:{PORT}/api/health"),
        _box(f"配置查询: http://{_display_host}:{PORT}/api/config"),
        _box(f"CSV数据: http://{_display_host}:{PORT}/api/csv"),
        _box(f"延迟测试: http://{_display_host}:{PORT}/?t=时间戳 (HEAD/GET)"),
        _SEP,
        _box(f"当前模式: {model_info}"),
    ]
    if model_detail:
        _banner_lines.append(_box(f"  {model_detail}"))
    _banner_lines += [
        _box(f"思考模式: {'✅ 已启用' if ENABLE_REASONING else '❌ 未启用'}"),
        _box(f"多选题思考: {'✅ 自动启用' if AUTO_REASONING_FOR_MULTIPLE else '❌ 关闭'}"),
        _box(f"图片题思考: {'✅ 自动启用' if AUTO_REASONING_FOR_IMAGES else '❌ 关闭'}"),
        _box("支持题型: 单选、多选、判断、填空"),
        _SEP,
        _box(f"💡 旧版HTML: http://{_display_host}:{PORT}/config_legacy"),
        _BOT,
    ]
    print("\n".join(_banner_lines))
